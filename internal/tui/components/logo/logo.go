...
func SmallRender(width int) string {
	...
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("Supernova", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "Supernova"
	if remainingWidth > 0 {
